//! The MCP server for the Getaway referee. A faithful copy of the chess warden's
//! `mcp.rs`, typed to `Match<Getaway>`: a SINGLE bearer-gated `/mcp` endpoint
...
//! road (e.g. `drive:highway`) instead of a UCI move.
...
use crate::getaway::Getaway;
use aiwars_mcp_warden::game::Match;

/// Shared, mutex-guarded match state shared across all agents' requests.
pub type SharedMatch = Arc<Mutex<Match<Getaway>>>;

/// `make_move` tool arguments.
#[derive(Debug, Deserialize, JsonSchema)]
pub struct MakeMoveArgs {
    /// The road to drive, one of your legal moves (e.g. "drive:highway",
    /// "drive:cross_st", "drift:backalley").
    pub mv: String,
...
impl RefereeHandler {
    #[tool(
        description = "Get the current chase state: your position/heat, the rival, the leader the chopper is on, status, winner, and your legal roads.",
    ...
    #[tool(
        description = "List the roads you may drive now (drive:highway / drive:cross_st / drift:backalley) and whether it is your turn, with the current ply.",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    ...
    #[tool(
        description = "Drive a road. Pass the ply you saw as expected_ply. Highways gain ground fast but spike HEAT (more under the chopper); alleys are slow and cool. Returns the new state on success; an error (no change) if illegal, out of turn, or stale.",
    ...
    #[tool(
        description = "Resign the chase (forfeit). Your rival is awarded the win.",
        annotations(read_only_hint = false, destructive_hint = false, open_world_hint = false)
    )]
...
impl ServerHandler for RefereeHandler {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "AIWars Getaway referee. Call get_state / legal_moves to observe, make_move to drive a \
             road (with the ply you saw), resign to forfeit. Reach the extraction boat before your \
             HEAT hits 100. Your seat is identified by your bearer token; you cannot act as your rival.",
        )
    }
}
