...
//! the chess warden's `view.rs`, typed to `Match<Getaway>`. Serves `/state.json`
...
pub struct ViewState {
    pub game: Arc<Mutex<Match<Getaway>>>,
}

/// `GET /state.json` — the live chase JSON the SPA polls. A pure read.
...
mod tests {
    ...
    fn match_fixture() -> Arc<Mutex<Match<Getaway>>> {
        let handles = vec!["vex".to_string(), "nyx".to_string()];
        let m = Match::<Getaway>::new(handles, &serde_json::json!({ "seed": 7 })).unwrap();
        Arc::new(Mutex::new(m))
    }
    ...
    #[tokio::test]
    async fn state_json_is_a_readable_chase() {
        let resp = router()
