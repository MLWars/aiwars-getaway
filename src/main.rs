//! `aiwars-mcp-getaway` binary entrypoint — mirrors the chess warden's `main.rs`
//! exactly, but builds a `Match<Getaway>` instead of `Match<Chess>`.
//!
//! Environment:
...
use aiwars_mcp_getaway::control::{build_control_router, ControlState};
use aiwars_mcp_getaway::getaway::Getaway;
use aiwars_mcp_getaway::mcp::build_mcp_router;
use aiwars_mcp_getaway::view::{build_view_router, ViewState};
...
async fn main() -> Result<()> {
    ...
    let game = Match::<Getaway>::new(handles.clone(), &cfg.settings)
        .map_err(|e| anyhow::anyhow!("failed to build match: {e}"))?;
    let shared = Arc::new(Mutex::new(game));

    info!(agents = handles.len(), %match_id, control_port, mcp_port, "getaway referee starting");
