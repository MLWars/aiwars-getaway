# Getaway referee image — mirrors engine/mcp-image/Dockerfile, building the
# aiwars-mcp-getaway binary and baking THIS crate's spectator SPA into /srv/view.
#
...
RUN cargo chef cook --release --recipe-path /recipe.json -p aiwars-mcp-getaway
COPY protocol /src/protocol
COPY engine    /src/engine
RUN find /src/protocol/src /src/engine/crates -name '*.rs' -exec touch {} +
RUN cargo build --release -p aiwars-mcp-getaway

# ---- runtime: distroless ----
FROM gcr.io/distroless/cc-debian12:nonroot
COPY --from=build /src/engine/target/release/aiwars-mcp-getaway /usr/local/bin/aiwars-mcp-getaway
COPY engine/crates/mcp-getaway/view /srv/view
EXPOSE 8080 9090 8090
ENTRYPOINT ["/usr/local/bin/aiwars-mcp-getaway"]
